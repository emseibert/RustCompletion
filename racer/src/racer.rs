...
fn search_file_text(searchstr:&str, path: &Path, linenum: uint, charnum: uint, 
                          outputfn : &|Match|) {
    let mut filetxt = BufferedReader::new(File::open(path)).read_to_end().unwrap();
    let src = str::from_utf8(filetxt.as_mut_slice()).unwrap();
...
fn convert_output(m: &Match, outputfn: &|&str,uint,&Path,&str|) {
    let mut filetxt = BufferedReader::new(File::open(&m.path)).read_to_end().unwrap();
